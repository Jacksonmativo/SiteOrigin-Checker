...
try:
    from whois_checker import (
        WhoisChecker, calculate_domain_age, parse_creation_date
    )
except ImportError:
...
class TestWhoisChecker(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # ~7 years
        self.old_creation_date = (
            datetime.now() - timedelta(days=2555)
        )
        # ~6 months
        self.new_creation_date = (
            datetime.now() - timedelta(days=180)
        )
        # ~3 years
        self.medium_creation_date = (
    ...
    def test_who_dat_api_failure(self, mock_get):
        ...
        self.assertTrue(
            result is None or result.get('domain_age_years') == 0
        )
                ...
                "events": [
                    {
                        ...
                        "eventDate": "2018-05-20T10:15:30Z"
                    }
                ]
            })
        ]
    ...
    def test_domain_validation(self):
        ...
        for domain in valid_domains:
            ...
            if hasattr(self.checker, '_is_valid_domain'):
                self.assertTrue(self.checker._is_valid_domain(domain))
            else:
                self.assertTrue(True)

        for domain in invalid_domains:
            if hasattr(self.checker, '_is_valid_domain'):
                self.assertFalse(self.checker._is_valid_domain(domain))
            else:
    ...
    def test_rate_limiting_handling(self, mock_get):
        ...
        # (either None or an error dict)
        self.assertTrue(
            result is None or (isinstance(result, dict) and
                               'error' in result)
        )
...
class TestUtilityFunctions(unittest.TestCase):
    ...
    def test_calculate_domain_age_edge_cases(self):
        ...
        # Should return 0 for future dates
        self.assertEqual(future_domain, 0)
...
class TestIntegration(unittest.TestCase):
    ...
    def test_full_domain_analysis_flow(self):
        ...
        with patch('requests.get') as mock_get:
            ...
            self.assertIn('registrar', result)
            self.assertTrue(len(result['registrar']) > 0)
